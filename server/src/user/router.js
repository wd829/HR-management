...
router.post('/saveUserProfile', async (req, res) => {
    try {
        ...
        const update = {
            ...
            roomNo: values.roomNo, ipMsgId: values.ipMsgId, sex: values.sex, group: values.group, status: values.userStatus, note: values.note
        };
...
router.post('/getTeamMember', async (req, res) => {
    try {

        const { _id } = req.body

        const teamLeader = await UserModel.findById(toObjId(_id));

        const filter = teamLeader.teamNo;

        const teamMember = await UserModel.find({ teamNo: filter })

        const realTeamMember = teamMember.filter(({ deletedAt }) => !deletedAt)

        if (realTeamMember) {
            return res.send({ error: true, message: 'User found', data: realTeamMember });
...
router.post('/updateUserStacks', async (req, res) => {
    try {
        const { key, row } = req.body;
        ...
        const result = await UserModel.findByIdAndUpdate(toObjId(key), update, { new: true });

        if (result) {
            return res.send({ error: true, message: 'User found', data: result });
        }
        else {
            return res.send({ error: true, message: 'User not found', data: '' });
        }
    }
    catch (e) {
        return res.status(400).json({ message: e.message });
...
router.get('/getTeamUserName', async (req, res) => {
    try {
        ...
        for (let i = 0; i < allUsers.length; i++) {
            const fullName = allUsers[i].lastName + " " + allUsers[i].firstName;
            const teamName = allUsers[i].teamNo;
            userName.push({ teamname: teamName, username: fullName });
        }
...
router.get('/getAllGroupNames', async (req, res) => {
    try {
        ...
        for (let i = 0; i < allUsers.length; i++) {
            const fullName = allUsers[i].lastName + " " + allUsers[i].firstName;
            const group = allUsers[i].group;
            userName.push({ group: group, username: fullName });
        }
...
router.get('/getBirthday', async (req, res) => {
    try {
              ...
              $match: {
                $expr: {
                  $and: [
                    ...
                    { $eq: [{ $dayOfMonth: { $toDate: '$birthday' } }, currentDateOfMonth] },
                  ],
                },
              },
            },
